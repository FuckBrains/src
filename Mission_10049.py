...
def web_submit(submit,chrome_driver,debug=0):
    ...
    if num_1 == 0:
        name = submit['Ukchoujiang']['lastname']+str(num_)        
    else:
        name = submit['Ukchoujiang']['lastname']+str(num_)+submit['Ukchoujiang']['firstname']
    ...
    sleep(1)
    phone = submit['Ukchoujiang']['homephone'].split('.')[0]
    phone = Submit_handle.get_uk_phone1(phone)
    for key in phone:
        chrome_driver.find_element_by_xpath('//*[@id="phone"]').send_keys(int(key))
    sleep(1)
